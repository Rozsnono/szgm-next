...
const UserContext = createContext({ user: {} as { _id: string, user: string, password: string, role: number, savedSubjects: any, savedTematiks: any, planedSubjects: any, savedPlannedSubjects: any } | null, setUser: (user: { user: string, password: string, role: number } | null) => { } });

export default UserContext;

export async function login(user: string, password: string) {
    await fetch("https://szgm-next-server.onrender.com/api/user?user=" + user + "&password=" + Coder(password)).then(res => res.json()).then(data => {
...
export async function register(user: string, password: string, role: number) {
    ...
    await fetch("https://szgm-next-server.onrender.com/api/user", { method: "POST", body: JSON.stringify({ user: user, password: Coder(password), role: role }), headers: { "Content-Type": "application/json" } })
}
