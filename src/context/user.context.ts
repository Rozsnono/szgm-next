...
export async function login(user: string, password: string) {
    await fetch("https://szgm-next-server-production.up.railway.app/api/user?user=" + user + "&password=" + Coder(password)).then(res => res.json()).then(data => {
...
export async function register(user: string, password: string, role: number) {
    ...
    await fetch("https://szgm-next-server-production.up.railway.app/api/user", { method: "POST", body: JSON.stringify({ user: user, password: Coder(password), role: role }), headers: { "Content-Type": "application/json" } })
}
