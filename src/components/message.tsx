...
export default function Message({ _id }: { _id: string }) {
  ...
  async function getMessage() {
    const res = await fetch("https://szgm-next-server.onrender.com/api/message/" + _id);
  ...
  async function createMessage() {
    if (message === '') return;
    const res = await fetch("https://szgm-next-server.onrender.com/api/message/" + _id, { method: 'PUT', body: JSON.stringify({ by: user.user, message: message }), headers: { "Content-Type": "application/json" } });
    const data = await res.json();
    setMessage('');
  ...
  async function reactToChat(reaction: string, selected: any) {
    const res = await fetch("https://szgm-next-server.onrender.com/api/message/reaction/"+_id, { method: 'PUT', body: JSON.stringify({ _id: selected._id, reaction: reaction }), headers: { "Content-Type": "application/json" } });
    setShowEmoji(null);
    const data = await res.json();
  }
