...
export default function Message({ _id }: { _id: string }) {
  ...
  async function createMessage() {
    if (message === '') return;
    const res = await fetch("/api/message/" + _id, { method: 'PUT', body: JSON.stringify({ by: user.user, message: message }), headers: { "Content-Type": "application/json" } });
    const data = await res.json();
    setMessage('');
  ...
  async function reactToChat(reaction: string, selected: any) {
    const res = await fetch("/api/message/reaction/655889877599d49c8afbba37", { method: 'PUT', body: JSON.stringify({ _id: selected._id, reaction: reaction }), headers: { "Content-Type": "application/json" } });
    setShowEmoji(null);
    const data = await res.json();
  }
