...
import PHYCS from "../../txts/jog.json";
...
export default function Home() {
    ...
    const phycs: any = useQuery('jog', getData);
...

            {
                !phycs.isLoading && phycs.data.length > queue ?
                    <QuestionCard key={queue} finished={(e) => {setQueue(0) }}
                        max={phycs.data.length} icon="book"
                        question={phycs.data[queue].question} number={queue + 1} answers={showAnswer?phycs.data[queue].answers:[]}
