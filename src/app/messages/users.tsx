...
export default function Users({ except }: { except: any[] }) {
    ...
    function template(item: any) {
        return (
            <div className="bg-gray-600 flex items-center border rounded-full gap-2 px-2 my-1">
    ...
    return (
        <main className="flex flex-col gap-2 mt-2 overflow-y-auto h-[53rem]">
            <div className="flex">
                ...
                <button onClick={() => { users.refetch() }} className="rounded-r-full bg-red-600 hover: hover:bg-red-800 duration-100 text-white px-2 pe-3"><i className="pi pi-search"></i></button>
            </div>
            <div className="flex">
                <div className="w-full rounded-l-full mx-auto px-1 bg-gray-600">
                    {
                        groupChat.map((user: any, index: number) => {
                            return (
                                <Chip className="bg-gray-600" key={index} label={user.user} removable template={template} />
                            )
                        })
                    }
                </div>
                <button onClick={newConversation} className="rounded-r-full bg-red-600 hover: hover:bg-red-800 duration-100 text-white px-2 pe-3"><i className="pi pi-users"></i></button>
            </div>
            {
                !users.isLoading &&
                users.data?.map((user: any, index: number) => {
                    return (
                        <div onClick={() => { setGroupChat([...groupChat, user]) }} key={index} className="w-full p-4 border-2 rounded-full border-red-700 group hover:bg-blue-700 bg-red-700 hover:text-white cursor-pointer duration-100">
                            <Avatar label={
                                (user.user).slice(0, 1).toUpperCase()
                            } className="mr-2 bg-blue-700 bg-red-700 text-white group-hover:bg-gray-600 group-hover:text-red-700 duration-100" size={"normal"} shape="circle" />
                            {
                                user.user
                            }
                        </div>
