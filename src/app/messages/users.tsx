...
export default function Users({ except }: { except: any[] }) {
    ...
    async function newConversation() {
        if (groupChat.length > 0) {
            const res = await fetch("https://sze-szerver.cyclic.app//api/message", { method: 'POST', body: JSON.stringify({ participants: [...groupChat.map((value) => { return { _id: value._id, name: value.user } }), { _id: user._id, name: user.user }], lastMessage: [] }), headers: { "Content-Type": "application/json" } })
            const data = await res.json();
